...
import { Star, ArrowRight, Users, Menu, X, Globe, ChevronDown, Phone, Mail, Shield, Check, Car, Heart, MessageCircle, Clock, Compass } from 'lucide-react';
import { CONTENT, WHATSAPP_LINK, FACEBOOK_PAGE, TIKTOK_PAGE, YOUTUBE_PAGE, GETYOURGUIDE_LINK, type Language } from './content';
        ...
        <div className="max-w-[1500px] mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <div className="hidden xl:flex flex-1 items-center justify-end gap-5 whitespace-nowrap">
            <div ref={langDropdownRef} className="relative">
              <button onClick={() => setLangDropdownOpen(!langDropdownOpen)} className="cursor-pointer flex items-center gap-1.5 hover:text-white transition-colors pl-2 font-semibold text-[13px] text-white/70 uppercase tracking-widest" aria-label="Switch language">
              ...
              <AnimatePresence>
                {langDropdownOpen && (
                  <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.95 }} transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }} className="absolute top-full right-0 mt-3 min-w-[160px] bg-[#111]/95 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden shadow-2xl z-50">
                    {(Object.keys(LANG_LABELS) as Language[]).map((code) => (
                      <button key={code} onClick={() => { setLang(code); setLangDropdownOpen(false); }} className={`cursor-pointer w-full text-left px-4 py-2.5 text-[13px] font-medium tracking-wide transition-all duration-200 flex items-center justify-between ${lang === code ? 'text-amber-300 bg-white/[0.05]' : 'text-white/60 hover:text-white hover:bg-white/[0.05]'}`}>
        ...
        <Routes location={location}>
          ...
          <Route path="/" element={
            ...
            >
              ...
              {/* ═══════ TAILOR-MADE / TOURS PERSONALIZADOS ═══════ */}
              ...
              {/* ═══════ ABOUT — Editorial split-screen (light palate-cleanser) ═══════ */}
